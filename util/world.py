...
class Textures:
    ...
    def __init__(self):
        ...
        self.sketch_color = numpy.zeros(
            [len(self.floor) + len(self.wall) + len(self.crate) + 2, 3])  # +2 b/c skybox & and tex 0
        for i in range(self.sketch_color.shape[0]):
            self.sketch_color[i] = numpy.array(
                [min(numpy.random.random() + 0.2, 1.0), min(numpy.random.random() + 0.2, 1.0),
...
class World:
    ...
    def __init__(self, control):
        ...
        control.limits = numpy.array(
            [numpy.iinfo('i').max, numpy.iinfo('i').max, numpy.iinfo('i').min, numpy.iinfo('i').min])
    ...
    def __constructWorld__(self, control):
        ...
        if control.type == 'box':
            self.__walls__.append(Wall(numpy.array([0.0, 0.0]), numpy.array([0.0, control.dim[1]]), control.dim[2],
                                       self.__textures__.wall[3 if control.wallmix else 0], control.wall_offset))
            self.__walls__.append(
                Wall(numpy.array([0.0, control.dim[1]]), numpy.array([control.dim[0], control.dim[1]]), control.dim[2],
                     self.__textures__.wall[0 if control.wallmix else 0], control.wall_offset))
            self.__walls__.append(
                Wall(numpy.array([control.dim[0], control.dim[1]]), numpy.array([control.dim[0], 0.0]), control.dim[2],
                     self.__textures__.wall[1 if control.wallmix else 0], control.wall_offset))
            self.__walls__.append(Wall(numpy.array([control.dim[0], 0.0]), numpy.array([0.0, 0.0]), control.dim[2],
                                       self.__textures__.wall[2 if control.wallmix else 0], control.wall_offset))
        ...
        # build star maze ( dim := [ <arms>, <arm_width>, <arm_length>, <arm_height> ] )
            ...
            for n in range(0, int(control.dim[0])):
                ...
                b = numpy.array([inner_r * math.cos((arm_dir - 0.5 * dir_step) * DEG2RAD) + math.cos(
                    arm_dir * DEG2RAD) * control.dim[2], \
                                 inner_r * math.sin((arm_dir - 0.5 * dir_step) * DEG2RAD) + math.sin(
                                     arm_dir * DEG2RAD) * control.dim[2]])
                c = numpy.array([inner_r * math.cos((arm_dir + 0.5 * dir_step) * DEG2RAD), \
                                 inner_r * math.sin((arm_dir + 0.5 * dir_step) * DEG2RAD)])
                d = numpy.array([inner_r * math.cos((arm_dir + 0.5 * dir_step) * DEG2RAD) + math.cos(
                    arm_dir * DEG2RAD) * control.dim[2], \
                                 inner_r * math.sin((arm_dir + 0.5 * dir_step) * DEG2RAD) + math.sin(
                                     arm_dir * DEG2RAD) * control.dim[2]])
                # arm walls (note: each complete arm gets a different texture in case of wallmix)
                self.__walls__.append(Wall(c, d, control.dim[3], self.__textures__.wall[
                    0 if not control.wallmix else n % len(self.__textures__.wall)], control.wall_offset))
                self.__walls__.append(Wall(d, b, control.dim[3], self.__textures__.wall[
                    0 if not control.wallmix else n % len(self.__textures__.wall)], control.wall_offset))
                self.__walls__.append(Wall(b, a, control.dim[3], self.__textures__.wall[
                    0 if not control.wallmix else n % len(self.__textures__.wall)], control.wall_offset))
                # next
                arm_dir += dir_step
        ...
        # build T maze ( dim := [ <vertical_length>, <vertical_width>, <horizontal_length>, <horizontal_width>, <wall_height> ] )
            ...
            # T walls
            self.__walls__.append(Wall(a, b, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 0],
                                       control.wall_offset))
            self.__walls__.append(Wall(b, c, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 1],
                                       control.wall_offset))
            self.__walls__.append(Wall(c, d, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 2],
                                       control.wall_offset))
            self.__walls__.append(Wall(d, e, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 3],
                                       control.wall_offset))
            self.__walls__.append(Wall(e, f, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 0],
                                       control.wall_offset))
            self.__walls__.append(Wall(f, g, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 1],
                                       control.wall_offset))
            self.__walls__.append(Wall(g, h, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 2],
                                       control.wall_offset))
            self.__walls__.append(Wall(h, a, control.dim[4], self.__textures__.wall[0 if not control.wallmix else 3],
                                       control.wall_offset))
        ...
        # build circle maze ( dim := [ <radius>, <segments>, <wall_height> ] )
            ...
            for w in range(0, int(control.dim[1])):
                a = numpy.array(
                    [control.dim[0] * math.cos(angle * DEG2RAD), control.dim[0] * math.sin(angle * DEG2RAD)])
                b = numpy.array([control.dim[0] * math.cos((angle + step) * DEG2RAD),
                                 control.dim[0] * math.sin((angle + step) * DEG2RAD)])
                self.__walls__.append(Wall(b, a, control.dim[2], self.__textures__.wall[
                    0 if not control.wallmix else int(w / (control.dim[1] / len(self.__textures__.wall)))],
                                           control.wall_offset))
                angle += step
        ...
        # build world from file ( dim := [ <file_name> ] )
            ...
            for l in world_file:
                ...
                par = l.split()
                if par[0] == 'floor':
                    self.__textures__.floor[0] = self.__textures__.index[par[1]]
                else:
                    self.__walls__.append(
                        Wall(numpy.array([float(par[0]), float(par[1])]), numpy.array([float(par[2]), float(par[3])]),
                             def_CUSTOM_HEIGHT, self.__textures__.index[par[4]],
                             self.__ctrl__.wall_offset))  # self.__textures__.wall[int(par[4])], self.__ctrl__.wall_offset ) )

    def __addObstacleWalls__(self, obstacle, boxmix=False, index=0):
        self.__walls__.append(Wall(numpy.array([obstacle[0], obstacle[3]]), numpy.array([obstacle[0], obstacle[1]]),
                                   self.__ctrl__.box_dim[2], self.__textures__.crate[0 if not boxmix else index],
                                   self.__ctrl__.wall_offset))
        self.__walls__.append(Wall(numpy.array([obstacle[2], obstacle[3]]), numpy.array([obstacle[0], obstacle[3]]),
                                   self.__ctrl__.box_dim[2], self.__textures__.crate[0 if not boxmix else index],
                                   self.__ctrl__.wall_offset))
        self.__walls__.append(Wall(numpy.array([obstacle[2], obstacle[1]]), numpy.array([obstacle[2], obstacle[3]]),
                                   self.__ctrl__.box_dim[2], self.__textures__.crate[0 if not boxmix else index],
                                   self.__ctrl__.wall_offset))
        self.__walls__.append(Wall(numpy.array([obstacle[0], obstacle[1]]), numpy.array([obstacle[2], obstacle[1]]),
                                   self.__ctrl__.box_dim[2], self.__textures__.crate[0 if not boxmix else index],
                                   self.__ctrl__.wall_offset))
    ...
    def validPosition(self, pos):
        ...
        for w in self.__walls__:
            ...
            if w.vec_from[1] < pos[1] and w.vec_to[1] >= pos[1] or w.vec_to[1] < pos[1] and w.vec_from[1] >= pos[1]:
                if (w.vec_from[0] + (pos[1] - w.vec_from[1]) / (w.vec_to[1] - w.vec_from[1]) * (
                        w.vec_to[0] - w.vec_from[0]) < pos[0]):
