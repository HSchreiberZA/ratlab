...
from tools.GUI import opengl_tex_text     as TXT
from tools.GUI import opengl_texture_load as TEX
...
class TickBox(Freezeable):
    ...
    def display(self):
        ...
        if self.tick:
            glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.tick_yay)
        else:
            glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.tick_nay)
        glBegin(GL_QUADS)
        glTexCoord2f(0.0, 0.0)
        glVertex2i(0, 0)
        glTexCoord2f(1.0, 0.0)
...
def display():
    ...
    if ctrl.act_param == 'box':
        glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.param_box);    posdim = (60, 280, 83, 143)
    elif ctrl.act_param == 'star':
        glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.param_star);   posdim = (60, 225, 144, 195)
    elif ctrl.act_param == 't':
        glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.param_T);      posdim = (60, 180, 152, 241)
    elif ctrl.act_param == 'circle':
        glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.param_circle); posdim = (60, 285, 108, 135)
    elif ctrl.act_param == 'custom':
        glBindTexture(GL_TEXTURE_2D, ctrl.tex_lib.param_custom); posdim = (60, 390, 123, 34)
    glBegin(GL_QUADS)
    glTexCoord2f(0.0, 0.0)
...
def generateScript():
    ...
    # get values
    if ctrl.act_param == 'box':
        env_param = 'dim ' + ctrl.box_fields[0].text + ' ' + ctrl.box_fields[1].text + ' ' + ctrl.box_fields[0].text
    elif ctrl.act_param == 'star':
        env_param = 'star_maze ' + ctrl.star_fields[0].text + ' ' + ctrl.star_fields[1].text + ' ' + ctrl.star_fields[
            2].text + ' ' + ctrl.star_fields[3].text
    elif ctrl.act_param == 't':
        env_param = 't_maze ' + ctrl.t_fields[0].text + ' ' + ctrl.t_fields[1].text + ' ' + ctrl.t_fields[
            2].text + ' ' + ctrl.t_fields[3].text + ' ' + ctrl.t_fields[4].text
    elif ctrl.act_param == 'circle':
        env_param = 'circle_maze ' + ctrl.circle_fields[0].text + ' ' + ctrl.circle_fields[1].text + ' ' + \
                    ctrl.circle_fields[2].text
    elif ctrl.act_param == 'custom':
        env_param = 'custom_maze ' + ctrl.custom_fields[0].text
