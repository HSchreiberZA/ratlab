...
def colorScale_ZA(value, val_min, val_max, zero_level=120,
                  scalar_only=False):  # ZA: zero align (preset zero_level in [0,1024])
    # checks
...
def sample_spatial(args):
    ...
    for i, s in enumerate(args):
        if s == 'all':
            ...
            ctrl.cfg.sample_dir.append((s, numpy.array([1, 0])))
        elif s == 's':
            ctrl.cfg.sample_dir.append((s, numpy.array([0, -1])))
        elif s == 'w':
            ctrl.cfg.sample_dir.append((s, numpy.array([-1, 0])))
            ...
            ctrl.cfg.sample_dir.append((s, numpy.array([numpy.sqrt(0.5), numpy.sqrt(0.5)])))
        elif s == 'se':
            ctrl.cfg.sample_dir.append((s, numpy.array([numpy.sqrt(0.5), -numpy.sqrt(0.5)])))
        elif s == 'sw':
            ctrl.cfg.sample_dir.append((s, numpy.array([-numpy.sqrt(0.5), -numpy.sqrt(0.5)])))
...
def sample_directional(args):
    ...
    if 'grid' in args:
        ...
        for i in grid:
            if ctrl.world.validPosition((i[0], i[1])):
                ...
                ctrl.cfg.sample_pos.append(
                    (i[0] - int(ctrl.setup.world.limits[0]), i[1] - int(ctrl.setup.world.limits[1])))
    ...
    # sample over custom/manual positions
        ...
        try:
            ...
            print(
                'Error! File \'./current_experiment/exp_map.png\' was not found.\nMake sure to create this file using the \'map\' parameter.')
            sys.exit()
        map_dat = map_img.load()
...
def display_sampler_spatial():
    ...
    print('Using sampling period %d (%d position checks per sampling run)' % (
    ctrl.cfg.sample_period, world_sample_size_x * world_sample_size_y))
    ...
    for r in range(0, len(ctrl.cfg.sample_dir)):
        ...
        if ctrl.cfg.sample_dir[r][1][0] <= 0 and ctrl.cfg.sample_dir[r][1][1] >= 0:
            dir_a = 180.0 - dir_a
        elif ctrl.cfg.sample_dir[r][1][0] <= 0 and ctrl.cfg.sample_dir[r][1][1] <= 0:
            dir_a = 180.0 + dir_a
        elif ctrl.cfg.sample_dir[r][1][0] >= 0 and ctrl.cfg.sample_dir[r][1][1] <= 0:
            dir_a = 360.0 - dir_a
            ...
            for pos_x in range(int(limits[0]), int(limits[2]), ctrl.cfg.sample_period):
                if ctrl.world.validPosition(numpy.array([pos_x, pos_y])):
                    ...
                    if ctrl.setup.rat.color == 'RGB':
                        ...
                        data = numpy.zeros((1, int(ctrl.setup.rat.fov[0]) * int(ctrl.setup.rat.fov[1])),
                                           dtype=numpy.float32)  # row-vector 'matrix'
                        for y in range(0, int(ctrl.setup.rat.fov[1])):
                            for x in range(0, int(ctrl.setup.rat.fov[0])):
                ...
                sys.stdout.write('\r' + '[SAMPLING][' + '=' * done + '-' * (50 - done) + ']~[' + '%d/%.2f' % (
                cnt, cnt / (world_sample_size_x * world_sample_size_y) * 100.0) + '%]')
                sys.stdout.flush()
        ...
        for k in range(0, ctrl.cfg.sample_order):
            ...
            for y in range(0, world_sample_size_y):
                for x in range(0, world_sample_size_x):
                    if numpy.isnan(sample_data_raw[k, x, y]):
                        ...
                        sample_data_scaled[x, y] = colorScale_ZA(sample_sign * sample_data_raw[k, x, y], local_min,
                                                                 local_max, scalar_only=True)

            # write plots
            if ctrl.cfg.verbose:
                filename = ctrl.cfg.file_prefix + 'sample_' + str(k + 1) + '_' + ctrl.cfg.sample_dir[r][0] + '[' + str(
                    local_min) + ';' + str(local_max) + ']' + '.png'
            else:
            ...
            numpy.savetxt('./current_experiment/sampling_values_raw/local/' + filename,
                          sample_data_raw[k] * sample_sign)
        ...
        for y in range(0, world_sample_size_y):
            for x in range(0, world_sample_size_x):
                if numpy.isnan(avg[x, y]):
                    ...
                    sample_data_scaled[x, y] = colorScale_ZA(sample_sign * avg[x, y], local_min, local_max,
                                                             scalar_only=True)

        # write plots
        if ctrl.cfg.verbose:
            filename = ctrl.cfg.file_prefix + 'avg_sample_' + str(k + 1) + '[' + str(local_min) + ';' + str(
                local_max) + ']' + '.png'
        else:
...
def main():
    ...
    if mode == 'spatial':
        ...
        for i, arg in enumerate(sys.argv):
            if arg == 'prefix':
                ctrl.cfg.file_prefix = sys.argv[i + 1]
            elif arg == 'setup':
                ctrl.cfg.setup_file = sys.argv[i + 1]
            elif arg == 'v':
                ctrl.cfg.verbose = True
    ...
    if mode == 'spatial':
        ...
        for folder in ['sampling_plots', 'sampling_values', 'sampling_values_raw']:
            ...
            if (os.path.isdir('./current_experiment/' + folder) == False):
                try:
                    os.mkdir('./current_experiment/' + folder)
                except:
                    fail = True

            if (os.path.isdir('./current_experiment/' + folder + '/local') == False):
                try:
                    os.mkdir('./current_experiment/' + folder + '/local')
                except:
                    fail = True

            if (os.path.isdir('./current_experiment/' + folder + '/average') == False):
                try:
                    os.mkdir('./current_experiment/' + folder + '/average')
                except:
                    fail = True
        ...
        if (os.path.isdir('./current_experiment/sampling_rad') == False):
            try:
                os.mkdir('./current_experiment/sampling_rad')
            except:
                fail = True

        if (os.path.isdir('./current_experiment/sampling_rad/local') == False):
            try:
                os.mkdir('./current_experiment/sampling_rad/local')
            except:
                fail = True

        if (os.path.isdir('./current_experiment/sampling_rad/average') == False):
            try:
                os.mkdir('./current_experiment/sampling_rad/average')
            except:
                fail = True
