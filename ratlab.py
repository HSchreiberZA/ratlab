...
import util.ratbot as ratbot
import util.opengl_text as text
...
def __keyboardSpecialPress__(key, x, y):
    ...
    # F12: take screenshot
        ...
        screenshot = glReadPixels(0, 0, ctrl.defines.window_width, ctrl.defines.window_height, GL_RGBA,
                                  GL_UNSIGNED_BYTE)
        im = img.frombuffer('RGBA', (ctrl.defines.window_width, ctrl.defines.window_height), screenshot, 'raw', 'RGBA',
                            0, 0)
        im.save(filename)
...
def main():
    ...
    if (os.path.isdir('./current_experiment/sequence') == False):
        try:
            os.mkdir('./current_experiment/sequence')
        except:
            return
    ...
    if 'duplex' in sys.argv:
        if (os.path.isdir('./current_experiment/sequence_color') == False):
            try:
                os.mkdir('./current_experiment/sequence_color')
            except:
                return
    ...
    for i, arg in enumerate(sys.argv):
        ...
        # world setup parameters
            ...
            ctrl.setup.world.wallmix = False
        elif arg == 'boxmix':
            ctrl.setup.world.boxmix = True
        elif arg == 'box':
            ctrl.setup.world.obstacles.append(
                [float(sys.argv[i + 1]), float(sys.argv[i + 2]), float(sys.argv[i + 3]), float(sys.argv[i + 4])])
        elif arg == 'dbox':
            ctrl.setup.world.obstacles.append(
                [float(sys.argv[i + 1]), float(sys.argv[i + 2]), float(sys.argv[i + 1]) + ctrl.setup.world.box_dim[0],
                 float(sys.argv[i + 2]) + ctrl.setup.world.box_dim[1]])
        elif arg == 'dim':
        ...
        # experiment configuration parameters
            ...
            ctrl.setup.rat.fov = numpy.array([55.0, 35.0])
        elif arg == 'arc':
            ctrl.setup.rat.arc = int(sys.argv[i + 1])
        elif arg == 'dev':
            ctrl.setup.rat.path_dev = float(sys.argv[i + 1])
        elif arg == 'mom':
            ctrl.setup.rat.path_mom = float(sys.argv[i + 1])
        elif arg == 'speed':
            ctrl.setup.rat.speed = float(sys.argv[i + 1])
        elif arg == 'loop':
            ctrl.setup.rat.path_loop = True
        elif arg == 'bias':
    ...
    print(
        'Use command line option \'-h\', \'h\', \'help\' or \'--help\' to display available\ncommand line parameters\n')
