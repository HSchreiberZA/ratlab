...
def printNetworkState(network):
    ...
    print('\tNetwork (%d layers) accepts' % len(network),
          ('COLOR' if network[0].in_channel_dim == 3 else 'GREYSCALE'), )
...
def initNetwork(wide_fov=True, color=True, noise=False):
    ...
    noise_node = mdp.nodes.NoiseNode(input_dim=exp_node.output_dim, output_dim=exp_node.output_dim,
                                     noise_args=(0, numpy.sqrt(0.05)))
...
def trainNetwork(network, batch_size=None, add_ICA_layer=False, frame_override=None, generic=False):
    ...
    ping = time.time()
    data = numpy.memmap('data_memmap', dtype=numpy.float32, mode='w+',
                        shape=(frames, frame_dim_x * frame_dim_y * raw_data_dim))
